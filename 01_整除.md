...
&emsp;&emsp;该函数有如下性质：
- 若 $x\leqslant y$ ，则 $\[x\]\leqslant\[y\]$   
- 若 $x=m+v$ ， $m$ 是整数， $0\leqslant v<1$ ，则 $m=\[x\],v=\\{x\\}$
- 对任意整数 $m$ 有 $\[x+m\]=\[x\]+m,\\{x+m\\}=\\{x\\}$ ， $\{x\}$ 是周期为1的周期函数。
- $\[x\]+\[y\]\leqslant\[x+y\]\leqslant\[x\]+\[y\]+1$ ，其中等号有且仅有一个成立。
-
...
- 对正整数 $m$ 有 $\[\frac{\[x\]}{m}\]=\[\frac{x}{m}\]$
- 不小于 $x$ 的最小整数是 $-\[-x\]$
- 设 $a,N$ 是正整数，那么正整数 $1,2,...,N$ 中被 $a$ 整除的正整数个数是 $\[N/a\]$
